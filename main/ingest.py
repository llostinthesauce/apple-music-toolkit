"""
merge_staging.py — merge a staging music folder into music library with zero extra disk space.
...
Handles the "Artist already exists" case by merging at the Album level:
  staging/Radiohead/Kid A/     →  Library/Radiohead/Kid A/   (new album added)
...
def merge(source: Path, dest: Path, dry_run: bool):
    ...
    audio_exts = {".m4a", ".mp3", ".flac", ".aiff", ".aif", ".wav"}

    for src_file in sorted(source.rglob("*")):
        if not src_file.is_file():
            continue
        if src_file.suffix.lower() not in audio_exts:
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Merge staging music folder into library (zero extra disk space).")
    parser.add_argument("--source", required=True, type=Path, help="Source folder (staging)")
    parser.add_argument("--dest",   required=True, type=Path, help="Destination folder (library root)")
